...
create table view_role (
    ...
    foreign key (role_idx) references role,
    foreign key (view_idx) references view
);
...
create table code_detail (
    code_detail_idx varchar(50) not null auto_increment,
    code_detail_en varchar(50),
    code_detail_ko varchar(150),
    master_code_idx varchar(50),
    primary key (code_detail_idx),
    foreign key (master_code_idx) references master_code
);
...
create table compiler (
    ...
    prjct_idx bigint,
    primary key (com_idx),
    foreign key (prjct_idx) references project
);
...
create table chat (
    ...
    foreign key (prjct_idx) references project,
    foreign key (mem_idx) references member
);
...
create table board (
    ...
    foreign key (code_detail_idx) references code_detail,
    foreign key (mem_idx) references member
);
...
create table answer (
    ...
    answer_report_num integer,
    board_idx bigint,
    mem_idx bigint,
    primary key (answer_idx),
    foreign key (board_idx) references board,
    foreign key (mem_idx) references member
);
...
create table answer_report (
    ...
    foreign key (answer_idx) references answer,
    foreign key (mem_idx) references member
);
...
create table calendar_check (
    ...
    foreign key (cal_idx) references calendar,
    foreign key (mem_idx) references member
);
...
create table chatting_content (
    ...
    foreign key (mem_idx, prjct_idx) references chat
);
...
create table compiler_file (
    ...
    foreign key (com_idx) references compiler
);
...
create table demand_content (
    ...
    foreign key (demand_idx) references demand
);
...
create table kanban_item (
    ...
    foreign key (kb_idx) references kanban,
    foreign key (mem_idx) references member
);
...
create table erd_column (
    ...
    foreign key (erd_idx) references erd
);
